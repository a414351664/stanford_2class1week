...
def backward_propagation_with_regularization(X, Y, cache, para, lambd):
    m = X.shape[1]
    W1 = para["W1"]
    W2 = para["W2"]
    W3 = para["W3"]
...
def compute_cost_with_regularization(a3, Y, para, lambd):
    ...
    logprobs = np.multiply(-np.log(a3), Y) + np.multiply(-np.log(1 - a3), 1 - Y)
    para_cost = lambd/(2*m) * (np.sum(np.square(W1)) + np.sum(np.square(W2)) + np.sum(np.square(W3)))
    cost = 1. / m * np.nansum(logprobs) + para_cost
    return cost
